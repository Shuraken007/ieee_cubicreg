...
if __name__ == '__main__':
    ...
    B = B * 1.j;
    S = np.random.sample(num_node-1)*(100+80j)
    #S = np.array([0.78203, 4.012712, -6.45068, -6.45086, -5.59356, -4.30349, 190.7697, -3.49422, -1.86383, 659.4273, -0.39498,
    ...
    easy.add_phase('iter', 'grad', 'value', 'M', 'e_min', 'time', 'vector',
                   phase='main_iter', file = 'cr_16_node_mode_' + steptype + '.txt', sep = ' | ',
                   iter  = '{:>6}',
                   grad  = '{:5.3e}',
                   value = '{:5.3e}',
                   M     = '{:11.5f}',
    ...
    easy.add_phase('iter', 'grad', 'value', 'M', 'e_min', 'time', 'vector',
                   phase='sub_M', file='cr_16_node_mode_' + steptype + '.txt', sep=' | ',
                   check_phase = 'main_iter',
                   M='{:11.5f}',
                   )
    start = time.time()
    cr = src.cubic_reg.CubicRegularization(x0, f, grad, hess, conv_tol=1e-9, L0 =0.00022, print_data = easy, stepmin =steptype, submaxiter=1000000000)
    ...
    #cr = src.cubic_reg.AdaptiveCubicReg(x0, f, grad, hess, 'exact', conv_tol=1e-6, maxiter = 1000000)
    #x_opt, intermediate_points, n_iter, flag = cr.adaptive_cubic_reg()
    with open('cr_16_node_mode_' + steptype + '.txt', 'a') as fi:
